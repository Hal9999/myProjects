...
public class DriverBigData extends Configured implements Tool
{
	...
	{
		int topKlimit = Integer.parseInt(args[0]);			// topK limit
		int countThreshold = Integer.parseInt(args[1]);		// minimum threshold to go to topK
		Path inputPath = new Path(args[2]);
		Path outputDir = new Path(args[3]);

		Configuration configuration = this.getConf();
		configuration.setInt("topKlimit", topKlimit);
		configuration.setInt("countThreshold", countThreshold);

		Job job = Job.getInstance(configuration);			// Define a new job
		job.setJobName("Exercise #3 - TopProductsPairs");	// Assign a name to the job
		...
		job.setCombinerClass(CombinerBigData.class);		// Set combiner class
